...
parser.add_argument('-JSON_PATH', default=f'data/AMIGOS_BPM_30sec_byperson.json', type=str)
parser.add_argument('-MEMMAP_PATH', default=f'data/AMIGOS_BPM_30sec_byperson.mmap', type=str)
...
parser.add_argument('-MAX_SEQ_LEN', default=61, type=int)
        ...
        for window in range(0, window_count-1):
            ...
            for val in hr:
                ...
                if j-1 > args.MAX_SEQ_LEN:
                    print(f'Max sequence length exceeded in person {person}, video {video}, window {window}')
                    break
