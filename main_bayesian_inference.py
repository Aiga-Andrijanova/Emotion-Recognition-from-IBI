...
parser.add_argument('-forward_passes', default=10, type=int)  # number of Monte Carlo forward passes
    ...
    for data_loader in [dataloader_train, dataloader_test]:
        ...
        if data_loader == dataloader_train:
            ...
            for name, param in model.named_parameters():
                # Set all layers except dropout to .eval
                if 'dropout' not in name:
                    param.requires_grad = False

            first_pass = True
            y_prims = torch.empty((1,))
            for forward_pass in range(args.forward_passes):
                first_batch = True
                y_prims_batches = torch.empty((1,))
                for x, y, lengths in data_loader:
                    y_prim = torch.softmax(model.forward(x, lengths), dim=1)
                    if first_batch:
                        y_prims_batches = y_prim
                        first_batch = False
                    else:
                        y_prims_batches = torch.cat((y_prims_batches, y_prim))

                if first_pass:
                    y_prims = y_prims_batches.unsqueeze(dim=0)
                    first_pass = False
                else:
                    y_prims = torch.cat((y_prims, y_prims_batches.unsqueeze(dim=0)))
